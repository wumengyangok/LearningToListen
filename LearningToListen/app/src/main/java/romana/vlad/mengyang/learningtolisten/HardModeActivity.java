...
public class HardModeActivity extends AppCompatActivity {
    ...
    public void run() {
        ...
        String numberMask = randomAvoid(numberArray, numberArray[number]);
        StringBuilder maskName = new StringBuilder();
        maskName.append(animalMask).append('_').append(colorMask).append('_').append(numberMask);
        ...
        correctNumber = new String(numberArray[number]);

        ImageView face = (ImageView) findViewById(R.id.animal_hard);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        grade = new Grade(playMode, Setting.Mode.CRAZY);
        run();
    }
    ...
    public void onClickIcon(View view) {
        ...
        if (iconName.contains(correctAnswer)&&iconName.contains(correctNumber)) {
            Toast.makeText(HardModeActivity.this, "Right", Toast.LENGTH_SHORT).show();
            grade.addGrade(difficulty, true);
            difficulty += 20;
            if (difficulty > 100)
                difficulty = 100;
        } else {
            Toast.makeText(HardModeActivity.this, "Wrong", Toast.LENGTH_SHORT).show();
